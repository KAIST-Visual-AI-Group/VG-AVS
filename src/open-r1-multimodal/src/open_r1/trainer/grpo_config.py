...
class GRPOConfig(TrainingArguments):
    ...
    # Per-step generation controls (for multi-turn)
    step_max_new_tokens: Optional[int] = field(
        default=32,
        metadata={
            "help": (
                "Maximum number of tokens to generate per step during multi-turn generation. "
                "This provides a hard cap on the per-step KV cache growth. If None, falls back to `max_completion_length`."
            )
        },
    )
